...
HeuristicTable::HeuristicTable(SharedEnvironment * _env, const std::shared_ptr<std::vector<float> > & map_weights, bool consider_rotation):
    ...
    main_heuristics = new float[state_size*loc_size];
    std::fill(main_heuristics,main_heuristics+state_size*loc_size,MAX_HEURISTIC);
};
...
void HeuristicTable::_compute_weighted_heuristics(
    ...
    if (!consider_rotation){
        ...
        // TODO(rivers): clean the unnecessary code here.
        ...
        for (int start_orient=0;start_orient<n_orientations;++start_orient){
            for (int loc_idx=0;loc_idx<loc_size;++loc_idx) {
                ...
                size_t idx=(start_loc_idx*loc_size+loc_idx)*n_orientations+start_orient;
                main_heuristics[idx]=cost;
            }
        }
    }
}
...
float HeuristicTable::get(int loc1, int loc2) {
    if (consider_rotation) {
        cerr<<"no valid to use this func if consider rotation. TODO: we can select the minimum of all orients if needed."<<endl;
        exit(-1);
    }
...
float HeuristicTable::get(int loc1, int orient1, int loc2) {
    ...
    size_t main_idx=(loc_idx1*loc_size+loc_idx2)*n_orientations+orient1;
    return main_heuristics[main_idx];
} 
...
void HeuristicTable::save(const string & fpath) {
    ...
    out.write((char *)main_heuristics,sizeof(float)*state_size*loc_size);
...
void HeuristicTable::load(const string & fpath) {
    ...
    in.read((char *)main_heuristics,sizeof(float)*state_size*loc_size);
