...
void GlobalManager::reset() {
    ...
    if (!async) {
        ...
        for (int i=0;i<neighbor_generators.size();++i) {
            neighbor_generators[i]->reset();
        }
...
void GlobalManager::update(Neighbor & neighbor, bool recheck) {
    ...
    if (neighbor.succ){
        ...
        if (recheck) {
            ...
            for (auto & aid: neighbor.agents) {
                old_sum_of_costs+=agents[aid].path.path_cost* (*agent_infos)[aid].cost_weight;
            }
...
bool GlobalManager::_run_async(TimeLimiter & time_limiter) {
    ...
    {
#ifndef NO_ROT
        sum_of_distances += HT->get(instance.start_locations[agent.id],instance.start_orientations[agent.id],instance.goal_locations[agent.id]) * (*agent_infos)[agent.id].cost_weight;
#else
        sum_of_distances += HT->get(instance.start_locations[agent.id],instance.goal_locations[agent.id]) * (*agent_infos)[agent.id].cost_weight;
#endif
    }
...
void GlobalManager::getInitialSolution(Neighbor & neighbor) {
    ...
    for (int i=0;i<agents.size();++i) {
        ...
        neighbor.sum_of_costs+=agents[i].path.path_cost * (*agent_infos)[i].cost_weight;
