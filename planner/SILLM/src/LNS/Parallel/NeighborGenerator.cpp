...
NeighborGenerator::NeighborGenerator(
    ...
    std::vector<Agent> & agents, std::shared_ptr<std::vector<LNS::AgentInfo> > agent_infos,
...
void NeighborGenerator::generate_parallel(const TimeLimiter & time_limiter) {
    ...
    if (num_threads == 1) {
        // Skip OpenMP parallelization when only using 1 thread to avoid conflicts with Ray
        generate(time_limiter, 0);
    } else {
